...
public class AddressPicker extends WheelPicker
{
	...
	public void setSelectedItem(String province, String city, String county)
	{
		...
		ArrayList<String> counties = countyList.get(selectedProvinceIndex).get(
				selectedCityIndex);
	...
	/**
	 * 隐藏省级行政区，只显示地市级和区县级。 设置为true的话，地址数据中只需要某个省份的即可 参见示例中的“city2.json”
	 *
	 * @param hideProvince
	 *            the hide province
	 */
	...
	/**
	 * 隐藏县级行政区，只显示省级和市级。 设置为true的话，hideProvince将强制为false 数据源依然使用“city.json”
	 * 仅在逻辑上隐藏县级选择框。
	 *
	 * @param hideCounty
	 *            the hide county
	...
	protected View makeCenterView()
	{
		...
		if (provinceList.size() == 0)
		{
			throw new IllegalArgumentException(
					"please initial options at first, can't be empty");
		}
		LinearLayout layout = new LinearLayout(activity);
		layout.setOrientation(LinearLayout.HORIZONTAL);
		layout.setGravity(Gravity.CENTER);
		final WheelView provinceView = new WheelView(activity);
		provinceView.setLayoutParams(new LinearLayout.LayoutParams(
				screenWidthPixels / 3, WRAP_CONTENT));
		...
		cityView.setLayoutParams(new LinearLayout.LayoutParams(
				screenWidthPixels / 3, WRAP_CONTENT));
		...
		countyView.setLayoutParams(new LinearLayout.LayoutParams(
				screenWidthPixels / 3, WRAP_CONTENT));
		...
		provinceView.setOnWheelViewListener(new WheelView.OnWheelViewListener()
		{
					...
					String item)
			{
				...
				cityView.setItems(cityList.get(selectedProvinceIndex),
						isUserScroll ? 0 : selectedCityIndex);
				// 根据地市获取区县
				countyView.setItems(countyList.get(selectedProvinceIndex)
						.get(0), isUserScroll ? 0 : selectedCountyIndex);
			}
		});
		cityView.setItems(cityList.get(selectedProvinceIndex),
				selectedCityIndex);
		cityView.setOnWheelViewListener(new WheelView.OnWheelViewListener()
		{
					...
					String item)
			{
				...
				// 根据地市获取区县
				countyView.setItems(
						countyList.get(selectedProvinceIndex).get(
								selectedCityIndex), isUserScroll ? 0
								: selectedCountyIndex);
			}
		});
		countyView.setItems(
				countyList.get(selectedProvinceIndex).get(selectedCityIndex),
				selectedCountyIndex);
	...
	public interface OnAddressPickListener
	{
		 ...
		 *            the city
		 * @param county
		 *            the county ，if {@hideCounty} is true，this is
		 *            null
